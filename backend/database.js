...
const dbPath = process.env.DB_PATH || path.join(__dirname, 'cfa.db');
...
function seedDatabase(db) {
  const existing = db.prepare('SELECT COUNT(*) as count FROM topics').get();
  ...
  for (const t of topics) insertTopic.run(t.name, t.description, t.color);

  const insertFlashcard = db.prepare('INSERT INTO flashcards (topic_id, question, answer, difficulty) VALUES (?, ?, ?, ?)');
  for (const f of flashcards) insertFlashcard.run(f.topic_id, f.question, f.answer, f.difficulty);
  ...
  for (const q of questions) insertQuestion.run(q.topic_id, q.question, JSON.stringify(q.options), q.correct_index, q.explanation, q.difficulty);

  db.save();
  console.log(`Seeded: ${topics.length} topics, ${flashcards.length} flashcards, ${questions.length} questions`);
}
